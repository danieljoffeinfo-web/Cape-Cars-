...
export default function FleetAdmin() {
  ...
  const booked    = FLEET.filter(c => c.status === 'Booked').length
  const service   = FLEET.filter(c => c.status === 'Service').length
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-wrap gap-3">
        {[
          ...
          { label: 'Total',     count: FLEET.length, cls: 'bg-neutral-100 text-neutral-700 border-neutral-200' },
        ].map(s => (
            ...
            <tbody className="divide-y divide-black/[0.04]">
              {FLEET.map(car => (
                <tr key={car.id} className="hover:bg-neutral-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] tracking-[0.15em] uppercase font-medium ${statusColor[car.status]}`}>
                      {car.status}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-right">
                    <button className="text-xs text-neutral-400 hover:text-neutral-900 transition-colors">Edit</button>
                  </td>
                </tr>
              ))}
            </tbody>
